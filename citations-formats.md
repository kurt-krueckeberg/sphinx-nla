# Citation Format and Philisophy

The philosophy of Evidence Explained is:

cite the source at the level that lets another researcher identify exactly what you used.

For Rentkammer case files the format would be:

```text
Niedersächsisches Landesarchiv, Abteilung Bückeburg [NLA BU], <case-file identifier>,
Actenstück <no.>, "<Designatio Actorum description>," in "<case-file title>,"
<case-file life span>.
```

For other documents without an entiry in a Desingation Actorum, the format
would be:

```text
NLA BU, <case-file identifier>, document headed "<heading from document>," in
"<case-file title>," <case-file life span>.
```
