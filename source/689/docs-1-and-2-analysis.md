# 1. What Document 1 (petition) shows {#_1_what_document_1_(petition)_shows}
...
## 2. What Document 2 (Amt Bückeburg's report) shows {#_2_what_document_2_(amt_bückeburg’s_report)_shows}
...
## 3. Clarifications of terms {#_3_clarifications_of_terms}
...
## Comments on the **NC** and **\"Nr Pr\"** numbering systems. {#_comments_on_the_nc_and_\"nr_pr\"_numbering_systems}

## The Numbering Systems {#_the_numbering_systems}

## 1. Why **two** numbering systems? {#_1_why_two_numbering_systems?}

- **NC number (e.g. NC 4381, NC 4487)** → This was essentially the **day-to-day filing number**. It told the clerk: **"this piece of paper is document number X in the chamber's current stack for this year."** It belonged to the **ingress/egress registry** --- a running counter of what came in, what went out, with the date. Think of it like a **mailroom stamp**.

<!-- -->

- **Nr pr. (protocol number)** → This was the entry number in the **protocol book** (**Protokollbuch**). A protocol book wasn't just a log. It was the **authoritative diary of decisions, actions, and references**. When a rescript was issued, or when a petition was ruled on, the essentials were copied into the protocol book. The number "Nr pr. 1274" means: **"If you want the official abstract of this matter, look it up under entry 1274 in this year's Rentkammer protocol volume."**
...
## 2. Why wasn't one number enough? {#_2_why_wasn’t_one_number_enough?}
...
## 3. What was a protocol book? {#_3_what_was_a_protocol_book?}
...
## Why was there only one \"Nr pr\" Number in the Case File? {#_why_was_there_only_one_\"nr_pr\"_number_in_the_case_file?}

Only one document in the case file, document 1, has a \"Nr pr\" number on it. There isn't any other document with a \"Nr pr\" number on it, despite the fact that \"KRes\" or \"Rescr.\", both meaning Kammer Rescript, appear seven times in the Acten Designation. \"KRes\" appears four times and \"Rescr.\" appears three times.

## 1. Why only *one* "Nr pr." in the surviving file? {#_1_why_only_one_nr_pr_in_the_surviving_file?}

- The **protocol number** was assigned once when the case was formally opened in the Rentkammer.

- Every subsequent document in the case (reports, cover slips, rescripts, replies) still *belonged* to that protocol entry --- but the number itself didn't need to be re-written on every single sheet.

- So, "Nr pr. 1274" = the *umbrella identifier* under which the whole Krückeberg loan matter was logged in the protocol book.

That's why you see it on Document 1 (the petition) but not again: it was the **anchor document** of the protocol entry.

## 2. Then why so many "KRes." and "Rescr." notes? {#_2_then_why_so_many_kres_and_rescr_notes?}

- Every time the Rentkammer issued an **order or instruction** (a *Reskript*), clerks annotated the document in the file with "KRes." or "Rescr." and the directive.

- These are **internal instructions** tied back to the same protocol entry.

- In the protocol book itself, each of those rescripts would have been noted under the **same protocol number (1274)** as follow-up actions.

So:

- Nr pr. = the umbrella case entry.

- KRes./Rescr. = individual executive actions, which were linked back to that one protocol number.

## 3. Why don't we see the protocol number on each Reskript? {#_3_why_don’t_we_see_the_protocol_number_on_each_reskript?}

Because the clerks didn't need to repeat it --- they were working inside the same bound dossier. The NC number (document traffic control) was enough to keep order within the case file. For institutional memory, you went to the protocol book and looked up Nr pr. 1274, where you would find:
...
- All seven Rentkammer orders (*KRes./Rescr.*) were written under that umbrella in the protocol book, but the number wasn't recopied onto every slip in the physical case file.

Would you like me to reconstruct how "Nr pr. 1274" might have looked in the Rentkammer's protocol book, line by line, using the information from Documents 1 and 2? That would make the distinction between the *file* and the *protocol* very concrete.
